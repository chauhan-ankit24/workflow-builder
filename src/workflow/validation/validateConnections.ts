...
import { detectCycles, findDisconnectedNodes } from '../../utils/graphAlgorithms';

export const validateConnections = (nodes: Node[], edges: Edge[]): ConnectionValidation[] => {
  ...
  // Build maps for incoming and outgoing edges
  ...
  // Validate each edge for existence
  edges.forEach(edge => {
  ...
  // Validate each node for connection constraints
  nodes.forEach(node => {
    ...
    if (node.type === 'start') {
      ...
      //     targetId: '',
      //     isValid: false,
      //     error: 'Non-start nodes must have exactly one incoming connection',
      //     code: 'INVALID_INCOMING_COUNT',
      //   });
      // }
      // if (outgoingCount !== 1) {
      //   validations.push({
      ...
      //     targetId: '',
      //     isValid: false,
      //     error: 'Non-start nodes must have exactly one outgoing connection',
      //     code: 'INVALID_OUTGOING_COUNT',
      //   });
      // }
    }
  });
